...
export default function DiscordActivity() {
  ...
  const togglePlay = useCallback(() => {
    ...
    if (!audio) return;
    // ควบคุมผ่านตัว audio โดยตรง แล้วปล่อยให้ event sync state
    if (audio.paused) {
  ...
  const selectTrack = useCallback(
    (index: number) => {
      ...
      if (!audio) return;
      if (!spotifyTrack) {
        // ถ้ากำลัง pause อยู่ ให้เล่นทันที
        if (audio.paused) {
          audio.play().catch((e) => console.error("Play failed:", e));
        }
      }
  ...
  const handleVolumeChange = useCallback((newVolume: number) => {
    setVolume(newVolume);
    setIsMuted(newVolume === 0); // FIX: muted อัตโนมัติเมื่อเลื่อนเป็น 0
  }, []);
  ...
  useEffect(() => {
    if (didAutoplay.current) return; // FIX: กัน StrictMode เรียกซ้ำ
    didAutoplay.current = true;

    const randomIndex = Math.floor(Math.random() * playlist.length);
    setCurrentIndex(randomIndex);

    // เริ่มต้นด้วย mute และตั้งค่าให้ effect อื่นจัดการเล่นให้
    setIsMuted(true);
    setIsPlaying(true); // ให้ effect ด้านล่างเป็นตัวเรียก audio.play()
  }, [playlist.length]); // FIX: ไม่มี volume ใน deps แล้ว
  ...
  useEffect(() => {
    if (spotifyTrack && isPlaying && audioRef.current) {
      ...
      // ไม่ต้อง setIsPlaying ทันที ปล่อยให้ event "pause" sync state
    }
  ...
  // Handle playback (ศูนย์กลางการ play/pause) + react เมื่อเปลี่ยนเพลง
  useEffect(() => {
    const audio = audioRef.current;
    if (!audio) return;
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-slate-950 via-purple-950 to-slate-950 relative overflow-hidden flex items-center justify-center p-4">
      ...
      {/* ใช้ src บน <audio> โดยตรง เพื่อให้ React จัดการเปลี่ยนเพลงตาม state */}
      <audio
        ref={audioRef}
